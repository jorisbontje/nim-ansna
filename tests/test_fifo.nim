...
suite "fifo":
  ...
  test "addAndRevise patham9":
    const fifoSize = 10
    var fifo = initFIFO(maxSize = fifoSize)
    ...
    for i in 0 ..< fifoSize:
      check fifo.queue[i].stamp.evidentalBase[0] == fifoSize - i
    ...
    check fifo.queue[fifoSize - i].stamp.evidentalBase[0] != fifoSize - i # as it was replaced

    let addedRet = fifo.queue[fifo.queue.high]
